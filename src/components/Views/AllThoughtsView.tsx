...
export default function AllThoughtsView({ status }: AllThoughtsViewProps): React.ReactNode {
  const { thoughts, isLoading, error, handleApprove, handleReject, handleTypeChange } = useThoughts({ status });
  ...
  if (thoughts.length === 0) {
    return (
      <div className="text-center text-gray-400 py-8">
        {status === 'pending' ? 'No thoughts to review' : 'No approved thoughts yet'}
      </div>
    );
  }
